...
public static class HUDEvents
{
    ...
    public static void TriggerInteractionFromInput(string interactionName, float duration, bool isCountdown)
    {
        ...
        Debug.Log($"[HUDEvents] 상호작용 트리거: {interactionName} ({duration}초, 카운트다운: {isCountdown})");
    }
    ...
    // 기존 메서드들 (하위 호환성)
    public static void TriggerInteractionStart(InteractionType type, float duration, bool isCountdown = false) => OnInteractionStarted?.Invoke(type, duration, isCountdown);
    public static void TriggerInteractionProgress(InteractionType type, float progress) => OnInteractionProgress?.Invoke(type, progress);
    public static void TriggerInteractionComplete(InteractionType type, bool success) => OnInteractionCompleted?.Invoke(type, success);
    public static void TriggerInteractionCancel() => OnInteractionCancelled?.Invoke();
    
    public static void TriggerPlayerStatusChange(int playerId, string status, string name) => OnPlayerStatusChanged?.Invoke(playerId, status, name);
    public static void TriggerPlayerConnectionChange(int playerId, bool isOnline) => OnPlayerConnectionChanged?.Invoke(playerId, isOnline);
}