...
import com.ggumtle.data.websocket.model.home.response.GetPartyParticipantsResponseDto
import com.ggumtle.data.websocket.model.home.response.toEvent
import com.ggumtle.data.websocket.model.social.request.CancelFriendRequestDto
import com.ggumtle.data.websocket.model.social.request.DeleteFriendDto
import com.ggumtle.data.websocket.model.social.request.GetProfileFriendDto
import com.ggumtle.data.websocket.model.social.response.CancelFriendRequestResponseDto
import com.ggumtle.data.websocket.model.social.response.DeleteFriendResponseDto
import com.ggumtle.data.websocket.model.social.response.GetProfileFriendResponseDto
import com.ggumtle.data.websocket.model.social.response.GetSentFriendRequestsResponseDto
...
import com.ggumtle.data.websocket.model.home.request.ChangeMonggingTypeDto
...
import com.ggumtle.data.websocket.model.home.response.ChangeMonggingTypeResponseDto
...
class WebSocketRepositoryImpl @Inject constructor(
    ...
    private suspend fun parseAndEmitEvent(message: WebSocketReceiveMessageDto) {
        try {
            ...
            when (message.type) {
                ...
                WebSocketMessageType.CHANGE_MONGGING_RESULT -> handleMessage<ChangeMonggingTypeResponseDto>(message){ it.toEvent() }
                else -> {
    ...
    override suspend fun deleteFriend(friendId: Long){
        val data = DeleteFriendDto(friendId)
        val message = WebSocketSendMessageDto(
    ...
    override suspend fun changeMonggingType(monggingId: Long) {
        val data = ChangeMonggingTypeDto(monggingId)
        val message = WebSocketSendMessageDto(
            type = WebSocketMessageType.CHANGE_MONGGING,
            data = json.encodeToJsonElement(data)
        )
        remoteDataSource.sendMessage(message)
    }
