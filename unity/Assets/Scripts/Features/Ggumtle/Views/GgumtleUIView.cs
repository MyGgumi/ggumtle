...
namespace Features.Ggumtle.Views
{
    ...
    public class GgumtleUIView : MonoBehaviour
    {
        ...
        private void UpdateProgressBarBasedOnState(float progress, bool isHoldProgress)
        {
            ...
            // Fake, Purified 상태에서는 프로그레스바와 UI 전체 숨기기
            bool shouldHideUI = currentState == Features.Ggumtle.Models.GgumtleState.Fake ||
            ...
            if (shouldHideUI)
            {
                ...
                if (_interactionUI != null)
                {
                    _interactionUI.style.display = DisplayStyle.None;
                }
                return;
            }
            else
