...
public class ResourceManager : MonoBehaviour
{
    ...
    public int currentLightCount = 0;
    ...
    public void SetLightCountInternal(int count)
    {
        ...
        if (_root == null)
        {
            Debug.LogError("[ResourceManager] _root가 null입니다. Initialize가 호출되지 않았습니다.");
            return;
        }
        ...
        Debug.Log($"[ResourceManager] 빛 개수 설정: {count}");
    }
    ...
    public bool AddInventoryItem(int slotNumber, int amount = 1)
    {
        ...
        if (slot != null && slot.AddItem(amount))
        {
            ...
            Debug.Log($"[ResourceManager] 슬롯 {slotNumber}에 {amount}개 추가, 총 개수: {slot.itemCount}");
            return true;
        }
        
        Debug.Log($"[ResourceManager] 슬롯 {slotNumber}에 추가 불가 (현재: {slot?.itemCount ?? 0}, 최대: {slot?.maxCount ?? 0})");
        return false;
    }
    ...
    {
        SetLightCount(currentLightCount);
