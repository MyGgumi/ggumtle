...
public class MonggingHUDExample : MonoBehaviour
{
    ...
    private void ShowKeyboardGuide()
    {
        ...
        Debug.Log(
            "1~3키: 꿈틀 진행도 | 4~7키: 상호작용 테스트 | 8키: UI 숨김 | 9키: 땅파기 | 0키: 상호작용시작"
        );
        ...
        Debug.Log(
            "NumPad1: 땅파기(3초) | NumPad2: 동료구조(5초) | NumPad3: 먹이주기(2초) | NumPad0: 취소"
        );
    }

    void Update()
    {
        if (hudController == null || Keyboard.current == null || !enableKeyboardTesting)
            return;
        ...
        {
            hudController.SetTimeRemaining(new TimeSpan(0, 5, 30));
            Debug.Log("[MonggingHUDExample] 시간을 5분 30초로 설정");
        }
    ...
    private void TogglePlayerRole()
    {
        ...
        PlayerRole newRole =
            currentRole == PlayerRole.Mongging ? PlayerRole.Mongdung : PlayerRole.Mongging;
    ...
    private void TriggerLightEvent()
    {
        ...
        else
        {
            ...
            hudController.SetLightCount(
                hudController.GetCurrentRole() == PlayerRole.Mongging
                    ? UnityEngine.Random.Range(5, 30)
                    : 0
            );
    ...
    private void TriggerDirectInteraction(string interactionName, float duration, bool isCountdown)
    {
        ...
        Debug.Log(
            $"[MonggingHUDExample] 직접 상호작용 트리거: {interactionName} ({duration}초, 카운트다운: {isCountdown})"
        );
    }
        ...
        while (isTimerRunning && gameTimeRemaining.TotalSeconds > 0)
        {
            ...
            if (isTimerRunning)
            {
                ...
                if (gameTimeRemaining.TotalMinutes <= 3 && gameTimeRemaining.TotalMinutes > 2.98)
                {
                    ...
                    gameTimeRemaining.TotalMinutes <= 1
                    && gameTimeRemaining.TotalMinutes > 0.98
                )
                {
