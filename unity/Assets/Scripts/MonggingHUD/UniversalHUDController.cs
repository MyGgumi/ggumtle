...
public class UniversalHUDController : MonoBehaviour
{
    ...
    private void EnsureManagerComponents()
    {
        if (gameTimeManager == null)
            gameTimeManager = gameObject.GetComponent<GameTimeManager>() ?? gameObject.AddComponent<GameTimeManager>();
        if (playerListManager == null)
            playerListManager = gameObject.GetComponent<PlayerListManager>() ?? gameObject.AddComponent<PlayerListManager>();
        if (resourceManager == null)
            resourceManager = gameObject.GetComponent<ResourceManager>() ?? gameObject.AddComponent<ResourceManager>();
        if (healthBarManager == null)
            healthBarManager = gameObject.GetComponent<HealthBarManager>() ?? gameObject.AddComponent<HealthBarManager>();
        if (interactionManager == null)
            interactionManager = gameObject.GetComponent<HUDInteractionManager>() ?? gameObject.AddComponent<HUDInteractionManager>();
    ...
    public void SetGgumtleProgress(int level, float progress) => gameTimeManager?.SetGgumtleProgress(level, progress);
    ...
    public void ShowBanner(string msg, float seconds = 2.2f) => notificationManager?.ShowBanner(msg, seconds);
    public void SetHealth(int currentHP, int maxHP = 100) => healthBarManager?.SetHealth(currentHP, maxHP);
    public void SetFaintState(bool isFainted, float reviveTime = 5f) => healthBarManager?.SetFaintState(isFainted, reviveTime);
    ...
    public bool UseInventoryItem(int slotNumber) => resourceManager?.UseInventoryItem(slotNumber) ?? false;
    public bool AddInventoryItem(int slotNumber, int amount = 1) => resourceManager?.AddInventoryItem(slotNumber, amount) ?? false;
    public void SetInventoryItemCount(int slotNumber, int count) => resourceManager?.SetInventoryItemCount(slotNumber, count);
    public int GetInventoryItemCount(int slotNumber) => resourceManager?.GetInventoryItemCount(slotNumber) ?? 0;
    ...
    private void OnFaintStateChanged(bool isFainted)
    {
        if (isFainted)
        {
            ...
            notificationManager?.ShowBanner("기절! 제한 시간 내 동료 몽깅이의 도움이 필요합니다.", 3f);
