...
| **환경변수** | `ACCESS_SECRET` (game-server와 동일한 BASE64 시크릿) |
...
| 옵션 | 단축 | 기본값 | 설명 |
|------|------|--------|------|
| `--host` | `-h` | localhost | 게임 서버 호스트 |
| `--port` | `-p` | 9000 | 게임 서버 포트 |
| `--players` | `-n` | 60 | 가상 플레이어 수 |
| `--duration` | `-d` | 300 | 테스트 시간 (초) |
...
- `feedGgumtle(ggumtleId, duration)` - 꿈틀이 먹이기
