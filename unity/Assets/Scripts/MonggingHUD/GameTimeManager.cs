...
public class GameTimeManager : MonoBehaviour
{
    ...
    public void SetTimeRemaining(TimeSpan t) 
    {
        ...
        if (_root == null)
        {
            Debug.LogError("[GameTimeManager] _root가 null입니다. Initialize가 호출되지 않았습니다.");
            return;
        }
    ...
    private void UpdateGgumtleUI()
    {
        ...
        if (ggumtleLevel >= 1 && ggumtleLevel <= 3)
        {
            ...
            if (currentMask != null) 
            {
                ...
                if (fill != null)
                {
                    ...
                    Debug.Log($"[GameTimeManager] {ggumtleLevel}단계: fill_{ggumtleLevel}of3 높이 {heightPercent}% 설정");
                }
            }
        }
        ...
        Debug.Log($"[GameTimeManager] 꿈틀 진행도 업데이트: {ggumtleLevel}단계 (고정 높이: {(ggumtleLevel >= 0 && ggumtleLevel <= 3 ? stageHeights[ggumtleLevel] : 0)}%)");
    }
