...
namespace Networks.Pipelines
{
    public class CommandDispatcher
    {
        private readonly Dictionary<
            PacketType,
            Action<Command, IChannelHandlerContext>
        > _commandMap = new();
                ...
                foreach (var method in methods)
                {
                    ...
                    if (handlerAttr != null)
                    {
                        _commandMap[handlerAttr.Type] = (command, ctx) =>
                            method.Invoke(null, new object[] { command, ctx });
                        Debug.Log(
                            $"CommandDispatcher - {type.Name}.{method.Name}에 CommandHandler({handlerAttr.Type}) 발견"
                        );
                    }
                }
            }
